...
/**
 * The program for evaluating the performance of parsers. The collected
 * statistics can be used to create learning curves and precision-recall curves.
 * All evaluation metrics (precision, recall and F-measure) are based on the
 * definition of correctness of translations, which is domain-specific.
 * Precision is the percentage of translations that are correct. Recall is the
 * percentage of input sentences that are correctly translated. F-measure is the
...
public abstract class ParserEvaluator {
	...
	/**
	 * Contains information about an input sentence and its MR translation. This
	 * information is passed to the evaluators to generate statistics.
	 * 
	 * @author ywwong
	 *
	 */
	protected static class Entry implements Comparable {
		...
		/**
		 * Sentences that have translations are ranked higher. For sentences that have
		 * translations, those with higher-scoring translations are ranked higher. The
		 * entry IDs ensure that the natural ordering is consistent with equals.
		 */
		@Override
	...
	private static TreeSet evaluate(PrintWriter out, Examples gold, String filename, int k)
		...
		// choose the top-scoring (correct) parse for each example, and create a new
		// entry for it
	...
	/**
	 * Summarizes the evaluation results using a particular evaluation metric. The
	 * results are usually statistical figures which are written to the specified
	 * character stream.
	 * 
	 * @param out
	 *            the character stream to write to.
	 * @param entries
	 *            sets of entries that contain information about input sentences in
	 *            each trial (e.g. whether the sentences are correctly translated);
	 *            there is one entry set for each trial.
	 */
	protected abstract void summarize(PrintWriter out, TreeSet[] entries);

	/**
	 * The main program for evaluating the performance of parsers. This program
	 ...
	 * <p>
	 * <ul>
	 * <li><code><u>config-file</u></code> - the configuration file that contains
	 * the current settings.</li>
	 * <li><code><u>output-file</u></code> - the output text file for storing the
	 * evaluation results.</li>
	 * <li><code><u>input-file</u></code> - the input XML file that contains parses
	 * to evaluate.</li>
	 * </ul>
	 * <p>
	 * If there are more than one input XML file, then each file represents a
	 * separate trial, and macroaveraging will be used to obtain the average
	 * statistics across all trials. Log messages are sent to the standard error
	 * stream, which can be captured for detailed error analysis.
	 * 
	 * @param args
	 ...
	 * @throws SAXException
	 *             if the XML parser throws a <code>SAXException</code> while
	 *             parsing.
	 * @throws ParserConfigurationException
	 *             if an XML parser cannot be created which satisfies the requested
	 *             configuration.
	 */
