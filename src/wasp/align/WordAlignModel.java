...
public abstract class WordAlignModel {
	...
	/**
	 * Creates and returns a new word alignment model as specified in the
	 * configuration file (via the key <code>Config.WORD_ALIGN_MODEL</code>).
	 * Currently, two type of word alignment models are recognized:
	 * <code>giza++</code> for the GIZA++ implementation of IBM models, and the
	 * <code>gold-standard</code> model which uses the word alignments given by the
	 * gold-standard augmented syntactic parse trees. <code>null</code> is returned
	 * if the given model type is not recognized.
	 * 
	 * @return a new word alignment model of the type specified in the configuration
	 *         file.
	 */
	...
	/**
	 * Trains the word alignment model. The best word alignments for each training
	 * example are then obtained and put into the <code>aligns</code> field of the
	 * training example.
	 * 
	 * @param examples
